...
def ui_core_loss_predict(m):
    ...
    if excitation == "Sinusoidal":
        ...
        with col1:
            ...
            Freq = st.slider("Frequency (kHz)", # Use kHz for front-end demonstration while Hz for underlying calculation
                             ...
                             step=config.streamlit.freq_step/1e3)   *1e3
            Flux = st.slider("Peak to Peak Flux Density (mT)", # Use mT for front-end demonstration while T for underlying calculation
                             config.streamlit.flux_min*1e3,  
                             config.streamlit.flux_max*1e3, 
                             config.streamlit.flux_max/2*1e3, 
                             step=config.streamlit.flux_step*1e3)   /1e3
            Bias = st.slider("DC Bias (mT)   (Coming Soon, Default as 0mT)", -300, 300, 0, step=int(1e7)) # Use step=Inf to disable this slider  #TBD
        ...
        with col1:
            core_loss_multiple(
                ...
                y1=[1e-3*loss(waveform='sine', algorithm='iGSE', material=material, freq=i, flux_p2p=Flux) for i in config.streamlit.core_loss_freq],
                y2=[1e-3*loss(waveform='sine', algorithm='ML', material=material, freq=i, flux_p2p=Flux) for i in config.streamlit.core_loss_freq],
                #y3=[loss(waveform='sine', algorithm='Ref', material=material, freq=i, flux_p2p=Flux) for i in config.streamlit.core_loss_freq],
        ...
        with col2:
            core_loss_multiple(
                ...
                y1=[1e-3*loss(waveform='sine', algorithm='iGSE', material=material, freq=Freq, flux_p2p=i) for i in config.streamlit.core_loss_flux],
                y2=[1e-3*loss(waveform='sine', algorithm='ML', material=material, freq=Freq, flux_p2p=i) for i in config.streamlit.core_loss_flux],
                #y3=[loss(waveform='sine', algorithm='Ref', material=material, freq=Freq, flux_p2p=i) for i in config.streamlit.core_loss_flux],
        ...
        core_loss_iGSE = loss(waveform='sine', algorithm='iGSE', material=material, freq=Freq, flux_p2p=Flux)/1e3
    ...
    if excitation == "Triangular":
        ...
        with col1:
            ...
            Freq = st.slider("Frequency (kHz)", 
                             ...
                             config.streamlit.freq_max/2/1e3, 
                             step=config.streamlit.freq_step/1e3)   *1e3
            Flux = st.slider("Peak to Peak Flux Density (mT)", 
                             config.streamlit.flux_min*1e3,  
                             config.streamlit.flux_max*1e3, 
                             config.streamlit.flux_max/2*1e3, 
                             step=config.streamlit.flux_step*1e3)   /1e3
        ...
        with col1:
            core_loss_multiple(
                st,
                x=[freq/1e3 for freq in config.streamlit.core_loss_freq],
                y1=[1e-3*loss(waveform='sawtooth', algorithm='iGSE', material=material, freq=i, flux_p2p=Flux, duty_ratio=Duty) for i in config.streamlit.core_loss_freq],
                y2=[1e-3*loss(waveform='sawtooth', algorithm='ML', material=material, freq=i, flux_p2p=Flux, duty_ratio=Duty) for i in config.streamlit.core_loss_freq],
        ...
        with col2:
            core_loss_multiple(
                st,
                x=[flux*1e3 for flux in config.streamlit.core_loss_flux],
                y1=[1e-3*loss(waveform='sawtooth', algorithm='iGSE', material=material, freq=Freq, flux_p2p=i, duty_ratio=Duty) for i in config.streamlit.core_loss_flux],
                y2=[1e-3*loss(waveform='sawtooth', algorithm='ML', material=material, freq=Freq, flux_p2p=i, duty_ratio=Duty) for i in config.streamlit.core_loss_flux],
        ...
        with col3:
            core_loss_multiple(
                ...
                y1=[1e-3*loss(waveform='sawtooth', algorithm='iGSE', material=material, freq=Freq, flux_p2p=Flux, duty_ratio=i) for i in config.streamlit.core_loss_duty],
                y2=[1e-3*loss(waveform='sawtooth', algorithm='ML', material=material, freq=Freq, flux_p2p=Flux, duty_ratio=i) for i in config.streamlit.core_loss_duty],
        ...
        core_loss_iGSE = loss(waveform='sawtooth', algorithm='iGSE', material=material, freq=Freq, flux_p2p=Flux, duty_ratio=Duty)/1e3
        core_loss_ML = loss(waveform='sawtooth', algorithm='ML', material=material, freq=Freq, flux_p2p=Flux, duty_ratio=Duty)/1e3
        st.header(f'Core Loss: {round(core_loss_iGSE,2)} kW/m^3 (by iGSE), {round(core_loss_ML,2)} kW/m^3 (by ML)')
    ...
    if excitation == "Trapezoidal":
        ...
        with col1:
            ...
            if DutyP>DutyN :
                BPplot=Flux/2 # Since Bpk is proportional to the voltage, and the voltage is proportional to (1-dp+dN) times the dp
                BNplot=-BPplot*((-1-DutyP+DutyN)*DutyN)/((1-DutyP+DutyN)*DutyP) # proportional to (-1-dp+dN)*dn
            else :
                BNplot=Flux/2 # proportional to (-1-dP+dN)*dN
        ...
        with col1:
            core_loss_multiple(
                st,
                x=[freq/1e3 for freq in config.streamlit.core_loss_freq],
                y1=[1e-3*loss(waveform='trapezoid', algorithm='iGSE', material=material, freq=i, flux_p2p=Flux, duty_ratios=duty_ratios) for i in config.streamlit.core_loss_freq],
                y2=[1e-3*loss(waveform='trapezoid', algorithm='ML', material=material, freq=i, flux_p2p=Flux, duty_ratios=duty_ratios) for i in config.streamlit.core_loss_freq],
        ...
        with col2:
            core_loss_multiple(
                st,
                x=[flux*1e3 for flux in config.streamlit.core_loss_flux],
                y1=[1e-3*loss(waveform='trapezoid', algorithm='iGSE', material=material, freq=Freq, flux_p2p=i, duty_ratios=duty_ratios) for i in config.streamlit.core_loss_flux],
                y2=[1e-3*loss(waveform='trapezoid', algorithm='ML', material=material, freq=Freq, flux_p2p=i, duty_ratios=duty_ratios) for i in config.streamlit.core_loss_flux],
        ...
        duty_ratios = [DutyP,DutyN,Duty0]
        core_loss_iGSE = loss(waveform='trapezoid', algorithm='iGSE', material=material, freq=Freq, flux_p2p=Flux, duty_ratios=duty_ratios)/1e3
        core_loss_ML = loss(waveform='trapezoid', algorithm='ML', material=material, freq=Freq, flux_p2p=Flux, duty_ratios=duty_ratios)/1e3
        st.header(f'Core Loss: {round(core_loss_iGSE,2)} kW/m^3 (by iGSE), {round(core_loss_ML,2)} kW/m^3 (by ML)')
    ...
    if excitation == "Arbitrary":
        ...
        core_loss_iGSE = loss(waveform='arbitrary', algorithm='iGSE', material=material, freq=Freq, flux=flux_list, frac_time=duty_list)/1e3
        core_loss_ML = loss(waveform='arbitrary', algorithm='ML', material=material, freq=Freq, flux=flux_list, frac_time=duty_list)/1e3
        st.header("Coming Soon! This Section is still under development.") #TBD
        st.header(f'Core Loss: {round(core_loss_iGSE,2)} kW/m^3 (by iGSE), {round(core_loss_ML,2)} kW/m^3 (by ML)')
